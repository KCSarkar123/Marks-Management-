...
//student_arr[k++]={st.rollno, st.name, st.marks1,st.marks2, st.marks3, st.marks4, 
st.marks5, st.totalmarks}; 
} 
fclose(fp); 
int i,j; 
for (i = 0; i < num- 1; i++) { 
...
for (i = 0; i < num; i++) { 
printf("%-10d %-30s %-20d %-20d %-20d %-20d %-20d %-20d\n", 
student_arr[i].rollno, student_arr[i].name, student_arr[i].marks1,student_arr[i].marks2, 
student_arr[i].marks3, student_arr[i].marks4, student_arr[i].marks5, 
student_arr[i].totalmarks); 
} 
} 
*/ 
void student(){ 
